...
const Dashboard = () => {
  ...
  const fetchActivationStatus = useCallback(async () => {
    ...
    setActivationStatus(row?.status ?? null);
    setActivationNote(row?.admin_note ?? null);
  ...
  }, [fetchActivationStatus, profile?.activated]);

  // Live updates when an admin approves/rejects the verification payment
  useEffect(() => {
    if (!user) return;

    const channel = supabase
      .channel(`activation-status-${user.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        (payload) => {
          const status = (payload.new as any)?.status ?? null;
          if (status) setActivationStatus(status);
          setActivationNote((payload.new as any)?.admin_note ?? null);
          // activation approval also flips users.activated
          refreshProfile();
        },
      )
      .subscribe();
  ...
  return (
    <div className="min-h-screen w-full relative">
      ...
      <main className="relative z-10 px-3 py-3 max-w-4xl mx-auto space-y-3">
        ...
        {/* Verification status — only after a withdrawal has been placed */}
        {hasWithdrawal && (profile?.activated || activationStatus) && (
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {profile?.activated ? (
                  ...
                  <Button size="sm" variant="outline" asChild>
                    <Link to="/activate">View</Link>
                  </Button>
                )}
              </div>

              {!profile?.activated && activationStatus === "rejected" && (
                <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-3">
                  <p className="text-xs font-semibold text-destructive">Reason for rejection</p>
                  <p className="text-sm text-muted-foreground mt-1">
                    {activationNote ||
                      "Your verification payment could not be confirmed. Please upload a genuine receipt of the exact amount."}
                  </p>
                </div>
              )}
            </CardContent>
      ...
      {/* Floating activation reminder */}
      {profile && hasWithdrawal && profile.rpc_purchased && profile.rpc_code && !profile.activated && !showTelegramBanner && (
        <Link
          to="/activate"
          className="fixed bottom-5 right-4 z-40 flex items-center gap-2 rounded-full bg-primary px-4 py-3 text-primary-foreground shadow-glow animate-pulse hover:animate-none"
        >
          <ShieldAlert className="w-5 h-5" />
          <span className="text-sm font-semibold">Activate your account</span>
        </Link>
      )}

      {/* Re-upload verification receipt */}
      <Dialog open={reuploadOpen} onOpenChange={setReuploadOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Re-upload verification receipt</DialogTitle>
          </DialogHeader>
          <ActivationPanel
            compact
            onSubmitted={() => {
